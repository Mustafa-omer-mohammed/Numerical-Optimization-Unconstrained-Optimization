function [xk, fk, gradfk_norm_seq, k, xseq, btseq , pcg_iter,fk_seq] = ...
...
gradfk_norm_seq = zeros(kmax+1, 1);
gradfk_norm_seq(1) = norm(gradfk);
...
%             pk = pcg(Hessfk_pk, -gradfk, epsilon_k, pcg_maxit);
            HessFx = Hessf(xk);
            [pk, ~, ~, iter] = gmres(HessFx, -gradfk, [],epsilon_k);
        ...
        otherwise
            ...
            HessFx = Hessf(xk);
            [pk, ~, ~, iter] = gmres(HessFx, -gradfk, [],epsilon_k);
%             [pk, ~, ~, iter]  = pcg(Hessf(xk), -gradfk, epsilon_k);
    ...
    % Update xk, fk, gradfk_norm , gradfk_norm_seq
    xk = xnew;
    fk = fnew;
    ...
    gradfk_norm_seq(k+1) = norm(gradfk);
    ...
    xseq(:, k) = xk;
    % Store  current fk in fk_seq
    fk_seq(:, k) = fk;
    % Store bt iterations in btseq
    btseq(k) = bt;
    
    % Store pcg_iter iterations for the Kth iteration 
    pcg_iter(k) = iter ;
end
...
fk_seq = fk_seq(1:k);
gradfk_norm_seq = gradfk_norm_seq(1:k);
end